...
# A caught-and-returned 500 never increments AWS/Lambda Errors, so the gateway's
# 5xx metric is alarmed as well — a 500 body stays visible either way.
resource "aws_cloudwatch_metric_alarm" "api_5xx" {
  alarm_name          = "${local.prefix}-api-5xx"
  alarm_description   = "5xx responses from the decomposed portal's API Gateway stage."
