...
- rollup rows for the drill's own slice (`ns=demo`, `run_date=2026-08-18` — both coordinates taken
...
- DAG edges, compared against the contract's required graph:
  ...
  `storage_cleanup`) — a dropped edge fails this check instead of surviving as prose here. The
  rollup's edges are generated by `dynamic "depends_on" { for_each = local.estate_rollup_leaf_tasks }`
  rather than written one per leaf, so the check resolves that local out of the `locals` block the
  task's `for_each` actually names and compares the resolved leaf set — dropping a leaf from the
  list, renaming the local, or deleting the rollup task all fail the check
