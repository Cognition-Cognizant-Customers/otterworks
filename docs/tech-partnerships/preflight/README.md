...
- Atlas: free-tier M0 in the existing project; database `ow_tp_cronbox_demo` with collections `documents` and `files` bootstrapped by the parent. Search indexes are owned by the cron-search unit.
